        ...
        <div class="hidden-xs hidden-sm" id="bs-example-navbar-collapse-1">
            {if $USE_GROUP_TABS}
                <ul class="nav navbar-nav">
                    ...
                    {foreach from=$groupTabs item=modules key=group name=groupList}
                        {* BEGIN PERSONAL_CABINET IEPARENTTAB FIX *}
                        {* capture name=extraparams assign=extraparams}parentTab={$group}{/capture *}
                        {capture name=extraparams assign=extraparams}parentTab={$smarty.foreach.groupList.index}{/capture}
                        {* END PERSONAL_CABINET IEPARENTTAB FIX *}
                        <li class="topnav">
            ...
            <div id="quickcreatetop" class="dropdown nav navbar-nav navbar-right">
                ...
                <ul class="dropdown-menu" role="menu">
                    {* PERSONAL_CABINET *}
                    {php}
                    $this->assign('canCreateAccounts', BeanFactory::newBean('Accounts')->ACLAccess('edit'));
                    $this->assign('canCreateContacts', BeanFactory::newBean('Contacts')->ACLAccess('edit'));
                    $this->assign('canCreateOpportunities', BeanFactory::newBean('Opportunities')->ACLAccess('edit'));
                    $this->assign('canCreateLeads', BeanFactory::newBean('Leads')->ACLAccess('edit'));
                    $this->assign('canCreateDocuments', BeanFactory::newBean('Documents')->ACLAccess('edit'));
                    $this->assign('canCreateCalls', BeanFactory::newBean('Calls')->ACLAccess('edit'));
                    $this->assign('canCreateTasks', BeanFactory::newBean('Tasks')->ACLAccess('edit'));
                    $this->assign('canCreateCases', BeanFactory::newBean('Cases')->ACLAccess('edit'));
                    {/php}
                    ...
                    {/if}
                    {if $canCreateCases}
                    <li><a href="index.php?module=Cases&action=EditView&return_module=Cases&return_action=DetailView">{$APP.LBL_QUICK_CASE}</a></li>
                    {/if}
                </ul>
            </div>
